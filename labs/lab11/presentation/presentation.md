...
![Редактирование /etc/default/grub](image/1.png){ width=70% }

## Обновление конфигурации GRUB2
![Выполнение grub2-mkconfig](image/2.png){ width=70% }

## Проверка меню загрузчика
![Меню GRUB после изменений](image/3.png){ width=70% }

## Загрузка в режим rescue.target
![Добавление параметра rescue.target](image/4.png){ width=70% }

## Просмотр системных модулей и среды
![Переменные среды](image/5.png){ width=70% }

## Загрузка в режим emergency.target
![emergency.target](image/6.png){ width=70% }

## Минимальный набор модулей
![Минимальные модули](image/7.png){ width=70% }

## Попытка сброса через rd.break (неудачно)
![Ошибки initramfs](image/8.png){ width=70% }

## Рабочий способ: init=/bin/bash
![init=/bin/bash](image/9.png){ width=70% }

## Успешная смена пароля
![Смена пароля root](image/10.png){ width=70% }

## Успешная загрузка после сброса
![Аутентификация root](image/11.png){ width=70% }
